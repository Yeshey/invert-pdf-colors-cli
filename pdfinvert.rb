...
def invert(file)
  ...
  IO::popen("unshare --user inkscape #{file} --pages=1 --export-filename=#{basename}.svg --pdf-font-strategy=substitute 2>&1") do |io|
  ...
  svg.map! { |line|
    ...
    line.gsub!(/"data:image\/(\w+?);base64,(.*)"/) { |match|
      ...
      # Remove border
      if ( $trimcolor != "" )
        IO::popen("magick #{imgname}.#{imgtype} -bordercolor \"##{$trimcolor}\" " +
      ...
      if ( convertimage?(pnr, imgctr) )
        if ( $colors == "" )
          IO::popen("magick #{imgname}.#{imgtype} -negate #{imgname}.#{imgtype} 2>&1") do |io|
            log += io.readlines.join
          end
        else
          $colororder.each { |color|
            fuzz = $colorrules[color][0]
            IO::popen("magick #{imgname}.#{imgtype} -fuzz #{fuzz}% " +
  ...
  IO::popen("unshare --user inkscape #{basename}_inv.svg --pages=1 --export-filename=#{outbasename}.pdf --pdf-font-strategy=substitute 2>&1") do |io|
    log += io.readlines.join
  end
...
elseIt's a part of the poppler-utils package, which you'll need to install.
